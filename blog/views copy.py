...
from .models import BlogPost, PostInteraction, SubCategory
...
def skate_section(request):
    ...
    posts = BlogPost.objects.filter(subcategory__in=skate_subcategories).order_by(
        "-published_date"
    )
...
def music_section(request):
    ...
    posts = BlogPost.objects.filter(subcategory__in=music_subcategories).order_by(
        "-published_date"
    )
...
def history_section(request):
    ...
    posts = BlogPost.objects.filter(subcategory__in=history_subcategories).order_by(
        "-published_date"
    )
